...
<div class="row">
	<div class="col-sm-12">
		<form action="<?php echo base_url('admin/users/add');?>" id="frm_User_New" class="form-horizontal"   method="post" onsubmit="return false;">
			<div class="form-group <?php if(form_error('user_name') != NULL){echo 'has-error';}else if(!empty($error_user)){echo 'has-error';}?>">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" placeholder="Enter Username" name="user_name" value = "<?php echo set_value('user_name');?>">
			...
			<div class="form-group <?php if(form_error('user_email') != NULL){echo 'has-error';}else if(!empty($error_mail)){echo 'has-error';}?>">
                ...
                <div class="col-sm-10">
                    <input name="user_email" class="form-control" placeholder="Enter Email" value="<?php echo set_value('user_email');?>" id="user_email" type="text">
			...
			<div class="form-group <?php if(form_error('user_fullname') != NULL){echo 'has-error';}?>">
                ...
                <div class="col-sm-10">
                    <input name="user_fullname" class="form-control" placeholder="Enter Fullname" value="<?php echo set_value('user_fullname');?>" id="user_fullname" type="text">
	                ...
	                <select name="role" class="form-control">
                       <?php if(!empty($role)){?>
                            <?php foreach($role as $value):?>
                                <option value="<?php echo $value['role_id'];?>" <?php if($value['role_id'] == 5){ echo 'selected = ""';} ?>><?php echo $value['role_name']?></option>
                            <?php endforeach;?>
                       <?php } ?>
                    </select>
	            </div>
			...
			<div class="form-group ">
                <label class="col-sm-2 control-label" for="subject">Public Profile</label>
                <div class="col-sm-10">
	                <input type="checkbox" checked="" name="profile" value="1">
	            </div>
            </div>
		</form>
	</div>
