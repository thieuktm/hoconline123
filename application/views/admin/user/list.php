...
<div class="row">
    <div class="col-sm-12">
    	<form action="<?php echo base_url('admin/users/bulk_action');?>" method="post" id="frm-bulk-action">
                ...
                <table  class="table table-bordered table-hover" >
                    <thead>
                        <tr>
                            ...
                            <th style="width: 30px; background: #3c8dbc; color: white;">Username</th>
                            <th style="width: 100px; background: #3c8dbc; color: white;">Fullname</th>
                            <th style="width: 30px; background: #3c8dbc; color: white;">Email</th>
                            <th style="width: 30px; background: #3c8dbc; color: white;">Role</th>
                            <th style="width: 30px; background: #3c8dbc; color: white;">Activation</th>
                    ...
                    <tbody id="result">
                        ...
                        <?php if(!empty($result)){?>
                            ...
                            <?php foreach($result as $value): ?>
                                <tr id="record-<?php echo $value['iduser'];?>">
                                    <td><?php echo $stt++ ;?></td>
                                    <td><input type="checkbox" name="id[]" value="<?php echo (int)$value['iduser'];?>"></td>
                                    <td><?php echo $value['username']?></td>
                                    <td><?php echo $value['user_fullname']?></td>
                                    <td><?php echo $value['user_email']?></td>
                                    <td><?php if(!empty($arr[$value['usergroup']])){echo $arr[$value['usergroup']];}?></td>
                                    <td>
                                        <?php if($value['user_activation'] == 1){ ?>
                                            <span id="change-<?php echo  $value['iduser'];?>"><a class="label label-success" href="javascript:change_status(<?php echo $value['iduser']; ?>,'unpublish');">Activation</a></span>
                                        <?php }else{ ?>
                                            <span  id="change-<?php echo  $value['iduser'];?>"><a class="label label-danger" href="javascript:change_status(<?php echo $value['iduser']; ?>,'publish')">Not Activation</a></span>
                                        <?php }?>
                                    </td>
                                    <td>
                                        <button class="btn btn-primary btn-xs" onclick="User_Copy('<?php echo (int)$value["iduser"] ?>');" type="button">
                                            <i class="fa fa-files-o"></i>
                                        </button>
                                        <button class="btn btn-info btn-xs" onclick="User_Edit('<?php echo (int)$value["iduser"] ?>');" type="button">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button class="btn btn-danger btn-xs <?php if($value["iduser"] == $this->session->userdata('session_user_id')){ echo "disabled";}?>" onclick="User_Delete('<?php echo (int)$value["iduser"] ?>');" type="button">
...
<script>
    ...
    function User_Edit(id){
        ...
        var url = '<?php echo base_url("admin/users/edit");?>'+'/'+id+'/'+curent_page;
            ...
            buttons:[
                        {
                            ...
                            click:function(m_id,data){
                                $(".content").mask("<?php echo 'Loading...'; ?>");
                                var url = '<?php echo base_url("admin/users/delete");?>'+'/'+id;
                                $.get(url,function(result) {
                                    if(result.error == 0){
