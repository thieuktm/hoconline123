        ...
        <div class="col-md-6">
            ...
            <div class="panel panel-flat">
                ...
                <table class="table datatable-html">
                    ...
                    <tbody>
                        <?php foreach ($price_vn as $key => $item) {?>
                            <tr>
                                <td><?=$item->id?></td>
                                <td><?=$item->name?></td>
        ...
        <div class="col-md-6">
            ...
            <div class="panel panel-flat">
                ...
                <table class="table datatable-html">
                    ...
                    <tbody>
                        <?php foreach ($price_eng as $key => $item) {?>
                            <tr>
                                <td><?=$item->id?></td>
                                <td><?=$item->name?></td>
                                <td class="text-center">
...
<script type="text/javascript">
    function editTitle(id,lang){
        $.ajax({
            dataType: "json",
            type:"POST",
            url:"<?=base_url('admin/landcate/price_edit'); ?>",
            data:{id:id,lang:lang},
            success: function(result){
                var html        = '<form class="form-horizontal">';
                    html        +=  '<div class="form-group">';
                    html        +=      '<label for="Title" class="col-sm-2 control-label"><?=$this->lang->line('price'); ?></label>';
                    html        +=      '<div class="col-sm-10">';
                    html        +=          '<input type="text" class="form-control" id="Title" placeholder="<?=$this->lang->line('price'); ?>" value="'+result.name+'" required>';
    ...
    function delTitle(id,lang){
        if (confirm("are you ready?")) {
            $.ajax({
                dataType: "json",
                type:"POST",
                url:"<?=base_url('admin/landcate/price_del'); ?>",
                data:{id:id,lang:lang},
                success: function(result){
                    if(result == 1){
    ...
    function saveTitle(){
        ...
        $.ajax({
            dataType: "json",
            type:"POST",
            url:"<?=base_url('admin/landcate/price_update'); ?>",
            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        <label for="Title" class="col-sm-2 control-label"><?=$this->lang->line('price'); ?></label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="Title" placeholder="<?$this->lang->line('price'); ?>" />
                        </div>
                    </div>
                    <div class="form-group">
...
<script>
    function addDataTitle(){
        ...
        $.ajax({
            dataType: "json",
            type:"POST",
            url:"<?=base_url('admin/landcate/price_add'); ?>",
